 ...
 * Structure-preserving skeleton loading — images only, no artificial delays
 */

document.addEventListener("DOMContentLoaded", () => {
  // Only auto-apply to images that are genuinely still loading.
  // Do NOT auto-apply to .glass-card or other static content —
  // that content is already in the DOM and paints immediately;
  // wrapping it just adds a fake delay.
  document
    .querySelectorAll(
      ".glass-card img, .hero-photo-card img, .about-image-wrapper img",
    )
    .forEach((el) => {
  ...
  class Boneyard {
    static init() {
      ...
      bones.forEach((element) => {
        ...
        if (!isImage) {
          // Non-image elements only get a skeleton if a real async
          // signal is provided — a data-boneyard-ready flag flipped
          // by actual fetch/render completion, not a timer.
          console.warn(
            "Boneyard: skipping non-image element without an async completion signal",
            element,
          );
          return;
        }
